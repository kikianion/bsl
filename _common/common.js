		...
		"columnDefs": [
			...
			{//*** sum pagu summary
				...
				"targets": [
					...
					parseInt(flipped_colMap['total_apbddes']),
					parseInt(flipped_colMap['total_apbdkab']),
					parseInt(flipped_colMap['total_apbdprov']),
					parseInt(flipped_colMap['total_apbn']),
				],
			},
			{//*** sum pagu summary
				"render": function( data, type, row ) {
			...
			{//
				"render": function( data, type, row ) {
					if(type=='display'){
						return data+' <button class="act-detail-jml-keg">+</button>';
					}
					else{
						return data;
					}
				...
				"targets": [
					parseInt(flipped_colMap['jml_keg']),
				],
			},
			{//
				"render": function( data, type, row ) {
					if(type=='display'){
						return data+' <button class="act-detail-jenis-kec">+</button>';
					}
					else{
						return data;
					}
				...
				"targets": [
					parseInt(flipped_colMap['jenis_kec']),
				],
			},
			{//
				"render": function( data, type, row ) {
					if(type=='display'){
						return data+' <button class="act-detail-jenis-bidang">+</button>';
					}
					else{
						return data;
					}
				...
				"targets": [
					parseInt(flipped_colMap['jenis_bidang']),
				],
			},
...
var htmlFaClassRenderer1= function(instance, td, row, col, prop, value, cellProperties){
	td.innerHTML = value;
	...
	if(value) td.innerHTML="<i class='fa "+value+"'></i>";

};

var htmlStackFaSpanRenderer1= function(instance, td, row, col, prop, value, cellProperties){
	td.innerHTML = value;
	...
	if(value) td.innerHTML="<div style='white-space: normal'>"+value+"</i>";
...
function hot_transform_change_cell2row(hotInstance, change, transformInfo){
	...
	//change row string to row int val
	for(var i=0; i<change.length; i++){
		change[i][0]=parseInt(change[i][0]);
	}
...
function reload_route(){
	var appElement = document.querySelector('[ng-app="sippdlmgApp"]');
...
function eb_download(data, fileName){
	...
	//$(".main-footer").append("<a href='data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64," + data+"'  class='cem' download=''>aaaaaaa</a>");
...
var eb_number_format="#,##0;-#,##0;;@";

var eb_format_left,
eb_format_left_white,
eb_format_left_bold, 
eb_format_left_italic, 
eb_format_left_bold_red, 
eb_format_rp, 
eb_format_rp_bold, 
eb_format_number, 
eb_format_number_bold, 
...
function create_excel_style(stylesheet){
	...
	eb_format_left_white=stylesheet.createFormat({
		alignment: {
			wrapText: true,
			horizontal: "left",
			vertical: "top",
	...
	eb_format_left_italic=stylesheet.createFormat({
		alignment: {
			wrapText: true,
			horizontal: "left",
			vertical: "top",
	...
	eb_format_number = stylesheet.createFormat({
		...
		border: eb_cell_border,     
		format:  eb_number_format,

	});                

	eb_format_number_bold = stylesheet.createFormat({
		alignment: {
			wrapText: true,
			horizontal: "right",
			vertical: "top",
...
function dtable_common_apply_filter(dtable_instance,colMap_arr, that, colIdx){
	...
	var val1=RegExp.escape(that.value);

	//if dtable use server side processing
	if(dtable_instance.init().bServerSide && dtable_instance.init().bServerSide ==true){
		dtable_instance.column( colIdx ).search( val1, false,false,true)
	} 
	else if(        
		colMap_arr[colIdx]=='u' ||
		colMap_arr[colIdx]=='b' ||
		...
		dtable_instance.column( colIdx ).search(val1 ? '^'+val1+'$' : '', true, false ,true)                
	}
	else if(
		colMap_arr[colIdx]=='jenis_u' || 
		colMap_arr[colIdx]=='jenis_b' || 
		colMap_arr[colIdx]=='jenis_p' || 
		colMap_arr[colIdx]=='jenis_k' 
	){
		if(that.value=='>0'){
			dtable_instance.column( colIdx ).search(val1 ? '^(?!0)' : '', true, false ,true)                
		}
		...
		colMap_arr[colIdx]=='total_pagu' || 
		colMap_arr[colIdx]=='total_pagu2' || 
		colMap_arr[colIdx]=='pagu' || 
		colMap_arr[colIdx]=='pagu2' ||

		colMap_arr[colIdx]=='apbddes' ||
		colMap_arr[colIdx]=='apbdkab' ||
		colMap_arr[colIdx]=='apbdprov' ||
		colMap_arr[colIdx]=='apbn'
	){
		if(that.value=='>0'){
			dtable_instance.column( colIdx ).search(val1 ? '^(?!Rp 0)' : '', true, false ,true)                
		}
		else if(that.value=='=0'){
			dtable_instance.column( colIdx ).search(val1 ? '^(Rp 0)' : '', true, false ,true)                
		}
	...
	else if(colMap_arr[colIdx]=='sumber_dana' ){
		//var break1=val1.replace(/ /gi,'|');
		var break1=val1;
		dtable_instance.column( colIdx ).search( "^.*("+break1+").*$", true,false,true)
	}
	//for filter, another column not defined above
	else{
		var break1=val1.replace(/ /gi,'|');
		var break1=val1.replace(/\\/gi,'');
		dtable_instance.column( colIdx ).search(break1 ? break1 : '', true, false ,true)                
	}
...
function convert_master_ubpk2int(master){
	var res=JSON.parse(JSON.stringify(master));
	for(var i=0; i<res.length; i++){
		res[i].kode_u=parseInt(res[i].kode_u);
		res[i].kode_b=parseInt(res[i].kode_b);
		res[i].kode_p=parseInt(res[i].kode_p);
		res[i].kode_k=parseInt(res[i].kode_k);
	}
...
function convert_master_ubp2int(master){
	var res=JSON.parse(JSON.stringify(master));
	for(var i=0; i<res.length; i++){
		res[i].kode_u=parseInt(res[i].kode_u);
		res[i].kode_b=parseInt(res[i].kode_b);
